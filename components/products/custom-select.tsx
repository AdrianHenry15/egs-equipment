...
const CustomSelect = forwardRef<HTMLDivElement, ICustomSelectProps>((props, ref) => {
    // Props
    const { title, value, fruitValue, options, handleFruitInputChange, handleInputChange, error } =
        props;
            ...
            return (
                <div className="flex flex-col w-full px-4 pb-4 lg:w-1/2">
                    ...
                    <button
                        ...
                        }  duration-300 ease-in-out transition-colors flex mt-4 w-min px-6 py-1 items-center border border-black whitespace-nowrap text-white rounded-md`}
        ...
        <div ref={ref} className="flex flex-col mt-4">
            ...
            <div className="flex flex-wrap">
                {options?.map((item, index) => {
                    ...
                    return (
                        <p
                            className={`${
                                selected === item ? "border-blue-500 border-2" : "border-zinc-500"
                            } hover:border-blue-500 border-2 duration-300 ease-in-out transition-colors bg-zinc-700 flex items-center justify-center text-white rounded-full m-2 px-4 cursor-pointer`}
                            key={index}
            ...
            {selected?.toLowerCase() === "other" && (
                <div className="flex flex-col w-full px-4 pb-4 lg:w-1/2">
                    ...
                    <input
                        value={value.toLowerCase() === "other" ? "" : value}
